-- Run as the database owner after replacing the two NOLOGIN role declarations with
-- environment-managed LOGIN roles in the deployment platform. No password is stored here.
DO $$
BEGIN
...
GRANT USAGE ON SCHEMA public TO egoagentos_runtime, egoagentos_auditor;
REVOKE ALL ON ALL TABLES IN SCHEMA public FROM egoagentos_runtime, egoagentos_auditor;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM egoagentos_runtime, egoagentos_auditor;

GRANT SELECT, INSERT, UPDATE ON tasks, approvals TO egoagentos_runtime;
GRANT SELECT, INSERT ON evidence, memories, idempotency TO egoagentos_runtime;
GRANT SELECT, INSERT ON audit_events TO egoagentos_runtime;
GRANT SELECT ON
    tasks, approvals, evidence, memories, audit_events, idempotency, schema_migrations
...
GRANT EXECUTE ON FUNCTION egoagentos_current_tenant()
    TO egoagentos_runtime, egoagentos_auditor;
